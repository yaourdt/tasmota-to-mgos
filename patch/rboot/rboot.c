...
#endif
#ifdef RBOOT_MV_SDKCONFIG_ENABLED
	ets_printf("rBoot Option: move esp init data\r\n");
#endif
...
#endif
#ifdef RBOOT_MV_SDKCONFIG_ENABLED
		romconf->sdk_config_source = flashsize;
#endif
...
#ifdef RBOOT_MV_SDKCONFIG_ENABLED
	if (romconf->sdk_config_source != flashsize) {
		ets_printf("Moving SDK config sectors before booting.\r\n");
		uint8 data[SECTOR_SIZE];
		for (int sec = 1; sec < 5; sec++) {
			if ( SPIRead(romconf->sdk_config_source - (SECTOR_SIZE * sec), data, SECTOR_SIZE) != 0 ) {
				break;
			}
			SPIEraseSector((flashsize / SECTOR_SIZE) - sec);
			SPIWrite(flashsize - (SECTOR_SIZE * sec), data, SECTOR_SIZE);
		}
		romconf->sdk_config_source = flashsize;
	}
#endif
